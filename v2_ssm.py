...
def run_single_ssm_synchronized(pdb, cfg, model, chains=['A', 'B', 'C']):
    """Run single SSM with synchronized mutations across chains"""
    model.eval()
    model.cuda()
    stime = time.time()

    # Get chain lengths and offsets
    chain_lengths = [len(pdb[f'seq_chain_{c}']) for c in chains]
    if not all(l == chain_lengths[0] for l in chain_lengths):
        raise ValueError("All chains must have the same length for synchronized mutations")
    
    # Create synchronized mutations
    ALPHABET = "ACDEFGHIKLMNPQRSTVWYX"
    mutations_list = []
    
    # For each position in the first chain
    for pos in range(chain_lengths[0]):
        wt_aa = pdb[f'seq_chain_{chains[0]}'][pos]
        if wt_aa == '-':
            continue
        ...
        for mut_aa in ALPHABET:
            if mut_aa != wt_aa:
                # Create same mutation for all chains
                chain_muts = []
                for chain in chains:
                    mut = Mutation(
                        position=[pos],  # same position in each chain
                        wildtype=[wt_aa],
                        mutation=[mut_aa],
                        ddG=0.0,  # placeholder
                        pdb_name=''
                    )
                    chain_muts.append(mut)
                mutations_list.append(chain_muts)

    # Run predictions for synchronized mutations
    all_ddgs = []
    for chain_muts in tqdm(mutations_list):
        ddgs_for_mut = []
        for chain, mut in zip(chains, chain_muts):
            # Create temporary PDB with single chain mutation
            tmp_pdb = deepcopy(pdb)
            tmp_pdb['mutation'] = mut
            
            # Featurize and predict
            batch = tied_featurize_mut([tmp_pdb])
            if batch is None:
                continue
            ...
            chain_M = chain_M.cuda()
            chain_encoding_all = chain_encoding_all.cuda()
            residue_idx = residue_idx.cuda()
            mut_positions = mut_positions.cuda()
            mut_wildtype_AAs = mut_wildtype_AAs.cuda()
            mut_mutant_AAs = mut_mutant_AAs.cuda()
            mut_ddGs = mut_ddGs.cuda()
            
            # Get prediction
            with torch.no_grad():
                ddg = model(X, S, mask, lengths, chain_M, chain_encoding_all, 
                          residue_idx, mut_positions, mut_wildtype_AAs, mut_mutant_AAs)
            ddgs_for_mut.append(ddg.item())
            
        # Average ddG across chains
        if ddgs_for_mut:
            avg_ddg = sum(ddgs_for_mut) / len(ddgs_for_mut)
            all_ddgs.append(avg_ddg)

    etime = time.time()
    print(f"ThermoMPNN synchronized predictions generated in {round(etime-stime, 2)} seconds.")
    
    return torch.tensor(all_ddgs), mutations_list
